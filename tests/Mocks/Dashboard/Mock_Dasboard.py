...
class MocksLogsDashboard(unittest.TestCase):
    ...
    def patch_get_available_json_file():
        return (
            ...
            patch(
                "Services.Metrics.Dashboard.Path.glob",
    ...
    def patch_sidebar_selectbox(mode: str):
        return patch(
            "Services.Metrics.Dashboard.st.sidebar.selectbox", return_value=mode
        )
    ...
    def patch_render_mode_handlers():
        return (
            patch(
                "Services.Metrics.DashboardGeneralLogs.GeneralData_Window.show_general_logs_window"
            ),
            patch(
                "Services.Metrics.DashboardAnalysisLogs.Analysis_Window.show_analysis_logs_window"
            ),
        )
