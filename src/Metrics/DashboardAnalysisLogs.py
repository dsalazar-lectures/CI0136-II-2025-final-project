...
def show_analysis_logs_window(
    ...
    logs_dataframes = _build_named_frames(available_files)
    if not logs_dataframes:
        st.warning(MSG_NO_VALID_JSON)
        st.stop()

    start_date, end_date = _combined_date_range(logs_dataframes)
...
def _build_named_frames(available_files: Iterable[Path]) -> List[Dict[str, pd.DataFrame]]:
    out: List[Dict[str, pd.DataFrame]] = []
    for f in available_files:
        df = load_logs_from_json(f)
        if not df.empty:
...
def _combined_date_range(items: List[Dict[str, pd.DataFrame]]) -> Tuple[pd.Timestamp, pd.Timestamp]:
