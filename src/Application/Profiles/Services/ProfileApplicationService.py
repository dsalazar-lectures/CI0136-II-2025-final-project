...
class ProfileApplicationService(IProfileApplicationService):
    ...
    def set_favorite_ingredients(self, user_id: int, favorites: List[str]) -> Optional[Profile]:
        favorites_norm = [f.replace("-", " ").strip().lower() for f in favorites if isinstance(f, str)]
